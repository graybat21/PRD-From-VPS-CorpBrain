# CorpBrain (SME용 실시간 데이터 클리닝 OS) PRD v0.4
- Owner 팀: 다온 & 회비서
- 최종 업데이트: 2026-04-22
...
- **AC 4 (실패/예외 방어 - 손상 문서 및 인식 불가):** Given 비밀번호가 걸려 있거나 해상도가 50dpi 미만인 물리적 손상/난독 문서가 업로드되었을 때, When 클리닝 엔진이 파싱을 시도하여 10초 이내에 정상 구조화가 불가능함을 판별하면, Then 무한 루프 추론 시도를 즉시 멈추고 "보안 해제 및 원본 해상도 상향 요망"이라는 구체적 오류 코드를 반환한 뒤 프로세스를 메모리 누수 없이 안전 종료(Graceful shutdown) 해야 한다.
...
| **Could Have** | **PII 오토 마스킹 컴플라이언스 (P4)** | 주민등록번호/기밀 데이터 자동 탐지 후 마스킹 처리 후 내보내어 B2B 세일즈 컴플라이언스 돌파. | **P1(결과 블록 처리 모듈)에 의존** |
...
- **보안/비용**:
  ...
  - 고객사가 지정한 PII(민감 정보)는 로컬 엔진 내에서도 추가적인 오토 마스킹/암호화 과정을 거쳐 이중 보안 적용.
